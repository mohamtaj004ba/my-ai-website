...
function renderWebsiteAnalytics(){
  ...
  const funnel=document.getElementById('websiteFunnel'),f=d.funnel||{},rows=[['Sessions',f.sessions||0],['Get Started',f.getStarted||0],['Form started',f.formStarted||0],['Checkout started',f.checkoutStarted||0],['Converted',f.converted||0]],base=Math.max(1,Number(f.sessions||0));
  if(funnel)funnel.innerHTML=rows.map(([label,n],i)=>'<div class="funnel-row"><div><b>'+esc(label)+'</b><span>'+Number(n).toLocaleString()+'</span></div><i style="width:'+Math.max(n?4:0,Math.min(100,(Number(n)/base)*100))+'%"></i><small>'+(i?Math.round(Number(n)/base*100)+'% of sessions':'Entry')+'</small></div>').join('');
  const intent=document.getElementById('websiteIntentMetrics');if(intent)intent.innerHTML=[['CTA clicks',Number(d.ctaClicks||0)],['Contact inquiries',Number(d.contactInquiries||0)],['Chat sessions',Number(d.chatSessions||0)],['Checkout starts',Number(d.checkoutStarts||0)],['Attributed MRR',financeMoney(d.attributedMrr||0)],['Engaged sessions',Number(d.engagedRate||0)+'%']].map(([k,v])=>'<div><b>'+esc(String(v))+'</b><span>'+esc(k)+'</span></div>').join('');
