...
function adminAiSnapshot(){
  ...
  return {
    ...
    readiness:{blockers:adminReadinessData?.blockers||[],requiredForLaunch:adminReadinessData?.requiredForLaunch||[],checks:Array.isArray(adminHealthData)?adminHealthData.slice(0,60):[]}
  };
}
...
function openConversation(id,{loadEarlier=false}={}){
  const x=conversationsData.find(v=>String(v.id)===String(id));if(!x)return;const changed=String(activeConversationId)!==String(id);if(changed)conversationMessageLimit=50;else if(loadEarlier)conversationMessageLimit+=50;activeConversationId=id;
...
function renderContacts(){
  ...
  const q=(document.getElementById('contactSearch')?.value||'').trim().toLowerCase(),typeFilter=document.getElementById('contactTypeFilter')?.value||'all',sort=document.getElementById('contactSort')?.value||'recent',all=buildContacts();
  const signature=JSON.stringify([q,typeFilter,sort]);if(signature!==contactLastFilterSignature){contactLastFilterSignature=signature;contactVisibleLimit=50}
...
function contactMessageSessions(c){
  const flat=c.conversations.flatMap(conv=>(Array.isArray(conv.messages)?conv.messages:[]).map((m,index)=>({
...
function contactMessageSessionKey(session){return [session.day,session.firstAt,session.lastAt,session.messages.length].join(':')}
function contactMessageSessionHtml(session,key){
  const count=session.messages.length,limit=Number(contactMessageSessionLimits[key]||50),visible=session.messages.slice(-limit),remaining=count-visible.length;
  return '<div class="contact-message-history-meta"><span>Showing '+visible.length+' of '+count+' messages</span>'+(remaining?'<button type="button" class="secondary-btn" data-load-contact-messages="'+esc(key)+'">Load '+Math.min(50,remaining)+' earlier</button>':'')+'</div><div class="contact-message-thread">'+visible.map(m=>'<div class="contact-message '+(m.dir==='out'?'out':'in')+'"><small>'+esc(m.dir==='out'?'CallerCore / team':m.who||'Caller')+' · '+esc(contactTimeLabel(m.at))+'</small><p>'+esc(m.text)+'</p></div>').join('')+'</div>';
}
...
function renderContactHistoryItem(e){
  ...
  if(e.filter==='message'){
    const s=e.session,key=contactMessageSessionKey(s),count=s.messages.length,start=contactTimeLabel(s.firstAt),finish=contactTimeLabel(s.lastAt),preview=s.messages[s.messages.length-1]?.text||'';
    return '<details class="contact-history-card message" data-contact-message-session="'+esc(key)+'"><summary><span class="history-icon message">✉</span><span class="history-summary-copy"><small>'+esc(start+(finish&&finish!==start?'–'+finish:''))+'</small><b>'+count+' message'+(count===1?'':'s')+'</b><p>'+esc(preview)+'</p></span><span class="history-chevron">⌄</span></summary><div class="contact-expand-body">'+contactMessageSessionHtml(s,key)+'</div></details>';
  }
  if(e.filter==='request'){
    const r=e.request;
...
function renderContactHistoryTimeline(c){
  ...
  const signature=c.key+'|'+contactHistoryFilter;if(signature!==contactHistoryLastSignature){contactHistoryLastSignature=signature;contactHistoryVisibleLimit=50;contactMessageSessionLimits={}}
  const visibleEvents=events.slice(0,contactHistoryVisibleLimit);
  const groups=new Map();
  for(const e of visibleEvents){const key=contactDateKey(e.at);if(!groups.has(key))groups.set(key,{at:e.at,items:[]});groups.get(key).items.push(e)}
  timeline.innerHTML=([...groups.values()].map(group=>'<section class="contact-history-day"><div class="contact-history-day-head"><b>'+esc(contactDateLabel(group.at))+'</b><span>'+group.items.length+' shown</span></div>'+group.items.map(renderContactHistoryItem).join('')+'</section>').join('')||'<div class="contact-history-empty">No '+esc(contactHistoryFilter==='all'?'activity':contactHistoryFilter+' activity')+' yet.</div>')+(events.length?'<div class="contact-history-load"><span>Showing '+visibleEvents.length+' of '+events.length+' activity items</span>'+(visibleEvents.length<events.length?'<button type="button" class="secondary-btn" id="loadMoreContactHistory">Load '+Math.min(50,events.length-visibleEvents.length)+' more</button>':'')+'</div>':'');
  ...
  document.getElementById('loadMoreContactHistory')?.addEventListener('click',()=>{contactHistoryVisibleLimit+=50;renderContactHistoryTimeline(c)});
  timeline.querySelectorAll('[data-load-contact-messages]').forEach(button=>button.addEventListener('click',()=>{const key=button.dataset.loadContactMessages,details=button.closest('[data-contact-message-session]');contactMessageSessionLimits[key]=Number(contactMessageSessionLimits[key]||50)+50;renderContactHistoryTimeline(c);const reopened=timeline.querySelector('[data-contact-message-session="'+CSS.escape(key)+'"]');if(reopened){reopened.open=true;reopened.scrollIntoView({block:'nearest'})}}));
...
function openContact(key){
  ...
  activeContactKey=c.key;contactHistoryFilter='all';contactHistoryLastSignature='';contactHistoryVisibleLimit=50;contactMessageSessionLimits={};
...
document.getElementById('contactTypeFilter')?.addEventListener('change',renderContacts);
document.getElementById('contactSort')?.addEventListener('change',renderContacts);
document.getElementById('loadMoreContacts')?.addEventListener('click',()=>{contactVisibleLimit+=50;renderContacts()});
...
async function refreshAdminView(view=currentAdminView(),{force=false,announce=true}={}){
  ...
  }else if(view==='agents'||view==='admin-automations'){
    add('fleet','/api/account?action=admin-fleet',60000,d=>{adminFleetData=d});
  }else if(view==='phones'){
...
function renderAdminFleet(){
  ...
  const aw=document.getElementById('adminAutomationGrid');if(aw){
    aw.innerHTML=visibleAutos.map(x=>{const state=x.total>0&&x.enabled===x.total?'covered':x.enabled>0?'partial':'off',tone=state==='covered'?'green':state==='partial'?'amber':'red',label=state==='covered'?'Fully covered':state==='partial'?'Partial':'None / off',flows=Array.isArray(x.workflows)?x.workflows:[],flowHtml=flows.length?'<div class="admin-automation-flow-list">'+flows.slice(0,5).map(f=>'<span class="'+(f.enabled!==false?'on':'off')+'"><i></i>'+esc(f.name||'Automation')+'</span>').join('')+(flows.length>5?'<small>+'+(flows.length-5)+' more</small>':'')+'</div>':'<div class="admin-automation-flow-list empty">No workflows configured</div>';return '<article class="panel integration-card admin-automation-card" data-automation-client="'+esc(x.workspaceId)+'"><div class="admin-automation-copy"><b>'+esc(x.workspaceName)+'</b><p>'+x.enabled+' enabled of '+x.total+' configured · '+esc(x.plan)+' plan</p>'+flowHtml+'</div><div class="admin-automation-card-action"><span class="tag '+tone+'">'+label+'</span><button class="admin-link" type="button">Open workspace →</button></div></article>'}).join('');
...
function renderWebsiteAnalytics(){
  ...
  set('webConversionRate',Number(d.conversions||0).toLocaleString());set('webConversions','Won during the selected period · not a same-visit rate');
  const summary=document.getElementById('websiteTrafficSummary');if(summary)summary.textContent=Number(d.pageViews||0).toLocaleString()+' page views · '+Number(d.pagesPerSession||0).toFixed(1)+' pages/session · '+Number(d.bounceRate||0)+'% bounce'+(d.coverage?.isRetentionCapped?' · Older history outside retained tracking may be excluded':'')+(d.coverage?.isIncomplete?' · '+Number(d.coverage.unavailableProspectRecords||0)+' indexed prospect records unavailable; conversion totals may be partial':'')+(Number(d.periodDays||0)&&Number(d.periodDays)!==Number(adminWebsiteDays)?' · Showing '+Number(d.periodDays)+'-day data; requested '+Number(adminWebsiteDays)+'-day range is not yet loaded':'')+(adminWebsiteLoadError?' · Refresh issue: '+adminWebsiteLoadError+' (showing previously loaded data)':'');
  ...
  const funnel=document.getElementById('websiteFunnel'),f=d.funnel||{},rows=[['Sessions',f.sessions||0],['Get Started',f.getStarted||0],['Form started',f.formStarted||0],['Checkout started',f.checkoutStarted||0],['Prospects won',f.converted||0]],base=Math.max(1,Number(f.sessions||0));
  if(funnel)funnel.innerHTML=rows.map(([label,n],i)=>'<div class="funnel-row"><div><b>'+esc(label)+'</b><span>'+Number(n).toLocaleString()+'</span></div><i style="width:'+Math.max(n?4:0,Math.min(100,(Number(n)/base)*100))+'%"></i><small>'+(i===rows.length-1?'Sales outcome (not session-linked)':i?Math.round(Number(n)/base*100)+'% of sessions':'Entry')+'</small></div>').join('');
  const intent=document.getElementById('websiteIntentMetrics');if(intent)intent.innerHTML=[['CTA clicks',Number(d.ctaClicks||0)],['Contact inquiries',Number(d.contactInquiries||0)],['Chat sessions',Number(d.chatSessions||0)],['Checkout starts',Number(d.checkoutStarts||0)],['MRR from period conversions',financeMoney(d.attributedMrr||0)],['Engaged sessions',Number(d.engagedRate||0)+'%']].map(([k,v])=>'<div><b>'+esc(String(v))+'</b><span>'+esc(k)+'</span></div>').join('');
  const barRows=(elId,items,labelKey,countKey='count',detailFn=null)=>{const el=document.getElementById(elId);if(!el)return;const max=Math.max(1,...items.map(x=>Number(x[countKey]||0)));el.innerHTML=items.map(x=>'<div class="analytics-rank-row"><div><b>'+esc(String(x[labelKey]||'Unknown'))+'</b><span>'+esc(detailFn?detailFn(x):Number(x[countKey]||0)+' sessions')+'</span></div><i><em style="width:'+Math.round(Number(x[countKey]||0)/max*100)+'%"></em></i></div>').join('')||'<p class="muted">No data in this period.</p>'};
...
function growthBucket(stage){stage=String(stage||'new');if(['new','inquiry'].includes(stage))return 'new';if(['follow_up','checkout_started'].includes(stage))return 'nurture';if(['qualified','proposal'].includes(stage))return 'qualified';if(stage==='converted')return 'converted';if(stage==='lost')return 'lost';return 'new'}
function prospectValue(p){return Number(p.monthlyValue||PLAN_DATA[p.plan]?.price||0)}
function prospectDue(p){
  if(['converted','lost'].includes(p.stage))return false;
  const now=Date.now(),target=Number(adminPlatformData?.leadFollowupHours||24)*3600000;
  if(Number(p.nextFollowUpAt||0))return Number(p.nextFollowUpAt)<=now;
  return now-Number(p.lastContactAt||p.lastRepliedAt||p.createdAt||now)>=target;
}
const growthColumnVisibleLimits=Object.create(null);
function renderGrowth(){
  const coverageNote=document.getElementById('growthCoverageNote');if(coverageNote)coverageNote.textContent=(adminWebsiteData.coverage?.isRetentionCapped?'Growth includes all retained prospects; older records outside retention may be excluded. ':'')+(adminWebsiteData.coverage?.isIncomplete?Number(adminWebsiteData.coverage.unavailableProspectRecords||0)+' indexed prospects are unavailable; counts and search results may be partial. ':'')+(Number(adminWebsiteData.periodDays||0)&&Number(adminWebsiteData.periodDays)!==Number(adminWebsiteDays)?'Website traffic charts show the last loaded '+Number(adminWebsiteData.periodDays)+'-day range, not the requested '+Number(adminWebsiteDays)+'-day range. ':'')+(adminWebsiteLoadError?'Growth refresh failed: '+adminWebsiteLoadError+' — showing previously loaded records.':'');
  ...
  const search=document.getElementById('growthSearch');if(search){search.value=growthSearch;search.oninput=()=>{growthSearch=search.value;if(growthSearch.trim())growthFilter='all';Object.keys(growthColumnVisibleLimits).forEach(key=>delete growthColumnVisibleLimits[key]);renderGrowth()}}
  ...
  const q=growthSearch.trim().toLowerCase(),matches=p=>!q||[p.name,p.business,p.email,p.phone,p.source,p.campaign,p.utmCampaign,p.plan,p.stage,p.owner,p.category,p.message,p.notes,...(Array.isArray(p.tags)?p.tags:[])].filter(Boolean).join(' ').toLowerCase().includes(q),filterOk=p=>growthFilter==='all'?true:growthFilter==='open'?!['converted','lost'].includes(p.stage):growthBucket(p.stage)===growthFilter,visible=all.filter(p=>filterOk(p)&&matches(p));
  ...
  if(pipeline)pipeline.innerHTML=groups.map(([key,label])=>{const rows=visible.filter(p=>growthBucket(p.stage)===key),limit=Number(growthColumnVisibleLimits[key]||12);return '<section class="growth-column" data-growth-drop="'+key+'"><div class="growth-column-head"><b>'+label+'</b><span>'+rows.length+'</span></div>'+rows.slice(0,limit).map(p=>'<article class="growth-card '+(prospectDue(p)?'due':'')+(prospectStagePending.has(String(p.id))?' pending':'')+'" draggable="'+(prospectStagePending.has(String(p.id))?'false':'true')+'" aria-busy="'+String(prospectStagePending.has(String(p.id)))+'" data-edit-prospect="'+esc(p.id)+'" data-growth-drag="'+esc(p.id)+'"><div><b>'+esc(p.business||p.name||p.email||'Prospect')+'</b><small>'+esc(p.name&&p.business?p.name:(p.email||p.phone||''))+'</small></div><div class="growth-card-meta"><span>'+esc((p.source||'website').replaceAll('_',' '))+'</span><span>'+esc(p.plan||'Plan unknown')+'</span></div><div class="growth-card-foot"><strong>'+financeMoney(prospectValue(p))+'/mo</strong><span>'+(prospectDue(p)?'Follow-up due':p.nextFollowUpAt?'Next '+new Date(p.nextFollowUpAt).toLocaleDateString():'No follow-up set')+'</span></div></article>').join('')+(rows.length? '':'<div class="growth-empty-column">Drop prospect here</div>')+(rows.length>limit?'<button type="button" class="secondary-btn compact growth-show-more" data-growth-more="'+esc(key)+'">Show '+Math.min(12,rows.length-limit)+' more · '+limit+' of '+rows.length+'</button>':'')+'</section>'}).join('');
  pipeline?.querySelectorAll('[data-growth-more]').forEach(button=>button.addEventListener('click',()=>{const key=button.dataset.growthMore;growthColumnVisibleLimits[key]=Number(growthColumnVisibleLimits[key]||12)+12;renderGrowth()}));
  ...
  pipeline?.querySelectorAll('[data-growth-drag]').forEach(card=>{
    card.addEventListener('dragstart',e=>{if(prospectStagePending.has(String(card.dataset.growthDrag))){e.preventDefault();return}card.dataset.wasDragged='1';card.classList.add('dragging');e.dataTransfer.effectAllowed='move';e.dataTransfer.setData('text/prospect-id',card.dataset.growthDrag)});
...
function companyDocumentIsExpired(record,now=Date.now()){
  const date=String(record?.expiresAt||'');
...
function renderDocuments(){
  ...
  if(list)list.innerHTML=visible.map(x=>'<article class="document-row"><div><b>'+esc(x.workspaceName)+'</b><small>'+esc(x.ownerEmail||'')+' · '+esc(x.plan||'')+'</small></div><div><span class="tag '+(x.status==='signed'?'green':x.status==='awaiting_signature'?'amber':'')+'">'+esc(x.status==='signed'?'Signed':x.status==='awaiting_signature'?'Awaiting signature':'Not sent')+'</span></div><div><b>'+esc(x.agreementVersion?'v'+x.agreementVersion:'—')+'</b><small>'+esc(x.signedName||'No signer yet')+'</small></div><div><span>'+(x.signedAt?new Date(x.signedAt).toLocaleDateString():'—')+'</span></div><div class="phone-actions">'+(x.downloadUrl?'<a class="admin-link" href="'+esc(x.downloadUrl)+'" target="_blank" rel="noopener">PDF</a>':'')+'<button class="admin-link" data-doc-client="'+esc(x.workspaceId)+'">Client</button></div></article>').join('');
  if(empty)empty.hidden=visible.length!==0;list?.querySelectorAll('[data-doc-client]').forEach(b=>b.addEventListener('click',()=>openAdminClient(b.dataset.docClient)));
  const standard=document.getElementById('standardDocumentList');if(standard)standard.innerHTML=(d.standard||[]).map(x=>'<a class="standard-document-card" href="'+esc(x.url)+'" target="_blank" rel="noopener"><span>↗</span><div><b>'+esc(x.name)+'</b><small>'+esc(x.type||'Legal')+'</small></div></a>').join('');
...
function inboxContactParts(){
  if(!currentInboxItem)return {name:'',email:''};
  const p=currentInboxItem.prospect||{};if(p.email)return {name:p.name||'',email:p.email};
  const messages=currentInboxItem.messages||[],inbound=[...messages].reverse().find(m=>m.direction==='inbound')||messages[0]||{},raw=String(inbound.from||'').trim(),match=raw.match(/^(.*?)\s*<([^>]+)>$/);
  return {name:(match?.[1]||'').replace(/^["']|["']$/g,'').trim(),email:(match?.[2]||raw.match(/[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}/i)?.[0]||'').trim().toLowerCase()};
}
function promoteInboxToGrowth(){
  if(!currentInboxItem)return;
  if(currentInboxItem.prospect?.id){showView('growth');openProspectModal(currentInboxItem.prospect.id);return}
  const contact=inboxContactParts(),subject=currentInboxItem.kind==='gmail'?(currentInboxItem.thread?.subject||'Email inquiry'):'Website inquiry';
  showView('growth');openProspectModal('',{name:contact.name,email:contact.email,source:currentInboxItem.kind==='gmail'?'Email':'Website',stage:'inquiry',notes:'Added from '+(currentInboxItem.kind==='gmail'?'Gmail':'website inbox')+' · '+subject});
}
...
function renderPlatformSettings(){
  ...
  set('platformDefaultSalesOwner',adminPlatformData.defaultSalesOwner||'');
  const af=document.getElementById('platformAutoFirstFollowup');if(af)af.checked=adminPlatformData.autoScheduleFirstFollowup!==false;
  const prefs=adminPlatformData.alertPrefs||{};
  [['platformAlertProspects','prospects'],['platformAlertBilling','billing'],['platformAlertOnboarding','onboarding'],['platformAlertClientCare','clientCare'],['platformAlertSystem','system']].forEach(([id,key])=>{const el=document.getElementById(id);if(el)el.checked=prefs[key]!==false});
...
async function savePlatformSettings(){
  ...
  const payload={expectedUpdatedAt:Number(adminPlatformData?.updatedAt||0),brandName,defaultAgentName:agentName,defaultTimezone:document.getElementById('platformTimezone')?.value||'America/Los_Angeles',supportEmail:email,defaultAfterHours:document.getElementById('platformDefaultAfterHours')?.value||'ai',analyticsWindowDays:Number(document.getElementById('platformAnalyticsWindow')?.value||30),adminRefreshSeconds:Number(document.getElementById('platformRefreshSeconds')?.value||60),leadFollowupHours:Number(document.getElementById('platformLeadFollowupHours')?.value||24),defaultSalesOwner:document.getElementById('platformDefaultSalesOwner')?.value||'',autoScheduleFirstFollowup:!!document.getElementById('platformAutoFirstFollowup')?.checked,alertPrefs:{prospects:!!document.getElementById('platformAlertProspects')?.checked,billing:!!document.getElementById('platformAlertBilling')?.checked,onboarding:!!document.getElementById('platformAlertOnboarding')?.checked,clientCare:!!document.getElementById('platformAlertClientCare')?.checked,system:!!document.getElementById('platformAlertSystem')?.checked},maintenanceMode:!!document.getElementById('platformMaintenanceMode')?.checked,launchGates:{previewIsolation:!!document.getElementById('launchGatePreviewIsolation')?.checked,disposableE2E:!!document.getElementById('launchGateDisposableE2E')?.checked,voiceLifecycle:!!document.getElementById('launchGateVoiceLifecycle')?.checked,productionEnvScope:!!document.getElementById('launchGateProductionEnvScope')?.checked,supportEmail:!!document.getElementById('launchGateSupportEmail')?.checked,businessTax:!!document.getElementById('launchGateBusinessTax')?.checked,legalReview:!!document.getElementById('launchGateLegalReview')?.checked}};
  if(btn){btn.disabled=true;btn.textContent='Saving…'}if(status){status.textContent='';status.className='form-status-line'}
  try{
...
document.getElementById('addCompanyDocument')?.addEventListener('click',()=>openCompanyDocumentModal());
document.getElementById('closeCompanyDocumentModal')?.addEventListener('click',closeCompanyDocumentModal);
document.getElementById('saveCompanyDocument')?.addEventListener('click',saveCompanyDocument);
document.getElementById('deleteCompanyDocument')?.addEventListener('click',deleteCompanyDocument);
document.getElementById('companyDocumentModal')?.addEventListener('click',e=>{if(e.target.id==='companyDocumentModal')closeCompanyDocumentModal()});
...
document.getElementById('deleteCampaignButton')?.addEventListener('click',deleteCampaign);
...
function renderAdminFinance(){
  ...
  const currentClients=adminClientsData.filter(x=>adminClientLifecycle(x)!=='past'),pastDueClients=currentClients.filter(x=>x.subscriptionStatus==='past_due'),collectionsRisk=pastDueClients.reduce((n,x)=>n+Number(PLAN_DATA[x.plan]?.price||0),0),expenseRows=Array.isArray(d.expenses)?d.expenses:[],vendorSpend=expenseRows.filter(x=>x.status!=='paused'&&x.frequency!=='one_time').reduce((n,x)=>n+(x.frequency==='annual'?Number(x.amount||0)/12:Number(x.amount||0)),0),today=new Date(),monthKey=today.getFullYear()+'-'+String(today.getMonth()+1).padStart(2,'0'),oneTimeSpend=expenseRows.filter(x=>x.frequency==='one_time'&&String(x.date||'').startsWith(monthKey)).reduce((n,x)=>n+Number(x.amount||0),0),history=Array.isArray(d.history)?d.history:[],last=history.at(-1),prior=history.at(-2),mrrDelta=Number(last?.revenue??d.mrr??0)-Number(prior?.revenue??last?.revenue??d.mrr??0);
  set('financeCollectionsRisk',financeMoney(collectionsRisk));set('financeCollectionsRiskMeta',(pastDueClients.length?pastDueClients.length+' past-due account'+(pastDueClients.length===1?'':'s'):'No past-due accounts')+' · Monthly plan value, not unpaid invoice balance');set('financeVendorSpend',financeMoney(vendorSpend));set('financeOneTimeSpend',financeMoney(oneTimeSpend));const verifiedHistory=history.filter(x=>x.source!=='preview_reconstruction');const verifiedDelta=verifiedHistory.length>1?Number(verifiedHistory.at(-1).revenue||0)-Number(verifiedHistory.at(-2).revenue||0):0;set('financeMrrMovement',verifiedHistory.length>1?((verifiedDelta>=0?'+':'')+financeMoney(verifiedDelta)):'—');set('financeMrrMovementMeta',verifiedHistory.length>1?'vs. prior recorded snapshot':'Waiting for recorded monthly history');
...
function setAdminExpenseMutationState(saving){
  adminExpenseSaving=!!saving;
  ['expenseNameInput','expenseVendorInput','expenseCategoryInput','expenseAmountInput','expenseFrequencyInput','expenseDateInput','expenseStatusInput','expenseNotesInput','saveExpenseButton','closeExpenseModal'].forEach(id=>{const el=document.getElementById(id);if(el)el.disabled=adminExpenseSaving});
  const button=document.getElementById('saveExpenseButton');if(button)button.textContent=adminExpenseSaving?'Saving…':'Save expense';
  const modal=document.getElementById('expenseModal');if(modal)modal.setAttribute('aria-busy',String(adminExpenseSaving));
}
async function saveExpense(){
  if(adminExpenseSaving)return;
  const modal=document.getElementById('expenseModal'),button=document.getElementById('saveExpenseButton'),status=document.getElementById('expenseFormStatus'),name=String(document.getElementById('expenseNameInput')?.value||'').trim(),amountInput=String(document.getElementById('expenseAmountInput')?.value??'').trim(),amount=Number(amountInput);
  ...
  try{const r=await fetch('/api/account?action=admin-finance-expense-save',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)}),data=await r.json().catch(()=>({}));if(!r.ok)throw new Error(data.error||'Could not save expense.');const expenses=adminFinanceData.expenses||[],index=expenses.findIndex(x=>String(x.id)===String(data.expense?.id));if(index>=0)expenses[index]=data.expense;else if(data.expense)expenses.push(data.expense);setAdminExpenseMutationState(false);closeExpenseModal();try{await refreshAdminView('finance',{force:true,announce:false})}catch(refreshError){renderAdminFinance();setAdminSyncState('error','Expense saved, but the finance view could not refresh. Refresh to verify the latest ledger.')}}
...
async function deleteExpense(id){
  ...
  try{const r=await fetch('/api/account?action=admin-finance-expense-delete',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:key,expectedUpdatedAt:Number(item.updatedAt||0)})}),data=await r.json().catch(()=>({}));if(!r.ok)throw new Error(data.error||'Could not delete expense.');adminFinanceData.expenses=(adminFinanceData.expenses||[]).filter(x=>String(x.id)!==key);try{await refreshAdminView('finance',{force:true,announce:false})}catch(refreshError){renderAdminFinance();setAdminSyncState('error','Expense deleted, but the finance view could not refresh. Refresh to verify the latest ledger.')}}
...
async function openAdminGlobalSearchResult(type,id,view){
  const input=document.getElementById('adminSearch');closeAdminGlobalSearch();if(input)input.value='';
  if(type==='ai'){openAdminAiGuide();return}
  if(type==='page'){showView(view);return}
  if(type==='client'){showView('clients');await openAdminClient(id);return}
  if(type==='gmail'){showView('inbox');await openInboxItem('gmail',id);setTimeout(()=>flashAdminSearchTarget(document.querySelector('[data-inbox-kind="gmail"][data-inbox-id="'+CSS.escape(id)+'"]')),100);return}
  ...
  if(type==='onboarding'){
    showView('onboarding');setTimeout(()=>{const row=document.querySelector('.onboarding-row[data-provision-id="'+CSS.escape(id)+'"]');if(row)openOnboardingDrawer(id,row);else flashAdminSearchTarget(document.getElementById('provisioningBoard'))},120);return;
  }
  if(type==='settings'){
    showView('platform-settings');setTimeout(()=>{const b=document.querySelector('[data-settings-jump="'+CSS.escape(id)+'"]');if(b)b.click()},100);return;
  }
  if(type==='agent'){showView('agents');setTimeout(()=>flashAdminSearchTarget(document.querySelector('[data-admin-agent-client="'+CSS.escape(id)+'"]')),100);return}
  if(type==='automation'){showView('admin-automations');setTimeout(()=>flashAdminSearchTarget(document.querySelector('[data-automation-client="'+CSS.escape(id)+'"]')),100);return}
...
document.addEventListener('keydown',event=>{if(event.key==='Escape'&&document.getElementById('adminClientDrawer')?.classList.contains('open'))closeAdminClient()});
...
async function refreshAdminCore(){
  ...
  if(sr.ok){adminSummaryData=(await sr.json()).summary||{};adminDataSyncAt.summary=Date.now()}
  if(cr.ok){adminClientsData=(await cr.json()).clients||[];adminDataSyncAt.clients=Date.now()}
  renderAdmin();
}
async function refreshAdminDashboard({button=null}={}){
  ...
  if(button){button.disabled=true;button.textContent='Refreshing…'}
  setAdminSyncState('syncing',button?'Refreshing all operational data…':'Syncing current view…');
  try{
    if(button){await refreshAdminCore();await loadAdminOps();await loadNotifications({silent:true})}
    else await refreshAdminView(currentAdminView(),{force:false,announce:false});
    updateAdminRefreshStamp();
  }catch(err){console.error('Admin refresh failed',err);setAdminSyncState('error','Some data could not sync')}
  finally{adminRefreshInFlight=false;if(button){button.disabled=false;button.textContent='Refresh'}}
}
