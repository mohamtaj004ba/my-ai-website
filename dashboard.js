...
async function deleteExpense(id){
  ...
  try{const r=await fetch('/api/account?action=admin-finance-expense-delete',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:key,expectedUpdatedAt:Number(item.updatedAt||0)})}),data=await r.json().catch(()=>({}));if(!r.ok)throw new Error(data.error||'Could not delete expense.');adminFinanceData.expenses=(adminFinanceData.expenses||[]).filter(x=>String(x.id)!==key);await refreshAdminView('finance',{force:true,announce:false})}
