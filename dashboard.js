...
const wrap=document.getElementById('planComparison');if(wrap)wrap.innerHTML=Object.entries(PLAN_DATA).map(([name,p])=>{const current=name===currentPlan;const list=name==='Starter'?['300 included minutes','1 location','Calls, leads & AI agent','Phone routing & support']:name==='Growth'?['600 included minutes','Up to 2 locations','Unified conversations & appointments','Automations & advanced analytics']:['Unlimited minutes','Up to 5 locations','Everything in Growth','API & webhooks'];return '<article class="plan-option '+(current?'current':'')+'"><span class="eyebrow">'+(current?'Your plan':'CallerCore '+name)+'</span><h3>'+name+'</h3><b>
...
const modal=document.getElementById('upgradeModal');function openModal(target){if(!modal||!PLAN_DATA[target])return;const t=PLAN_DATA[target];document.getElementById('modalTitle').textContent=(t.price>PLAN_DATA[currentPlan].price?'Upgrade to ':'Switch to ')+target;document.getElementById('modalCopy').textContent=target==='Pro'?'Unlock everything in Growth plus API and webhook access.':target==='Growth'?'Unlock conversations, appointment booking, automations, and advanced analytics.':'Use CallerCore core calling, lead capture, AI agent, routing, and support features.';const fs=Object.entries(FEATURE_INFO).filter(([k,v])=>target==='Pro'?true:v.tier===target);document.getElementById('modalFeatures').innerHTML=fs.map(([k,v])=>'<span>✓ '+v.title+'</span>').join('');document.getElementById('modalCta').textContent='Open Stripe billing';modal.classList.add('open');modal.setAttribute('aria-hidden','false')}
...
+p.price+'/mo</b><ul>'+list.map(x=>'<li>'+x+'</li>').join('')+'</ul><button class="'+(current?'secondary-btn':'primary')+'" '+(current?'disabled':'data-upgrade="'+name+'"')+'>'+(current?'Current plan':(p.price>d.price?'Upgrade to ':'Switch to ')+name)+'</button></article>'}).join('');bindUpgradeButtons()}
function setPlan(plan){currentPlan=plan;document.getElementById('planSelector')&&(document.getElementById('planSelector').value=plan);renderBilling();renderStages();renderOverviewUnlocks();renderEntitledApps()}
document.getElementById('planSelector')?.addEventListener('change',e=>setPlan(e.target.value));
