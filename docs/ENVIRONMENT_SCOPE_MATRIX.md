...
| `STRIPE_SECRET_KEY` | Live key; test mode is rejected | Test key only if Preview billing tests are enabled; live mode is rejected | Checkout and System Health fail closed on the wrong mode. |
| `STRIPE_PUBLISHABLE_KEY` | Matching live publishable key | Matching test publishable key | Secret and publishable modes must match or checkout fails before contacting Stripe. |
...
3. Confirm Production uses matching live Stripe secret/publishable keys; test or mixed modes are rejected.
4. Confirm Production `CALLERCORE_CHECKOUT_ENABLED` is still unset/false.
5. Confirm Preview KV is separate from Production KV before disposable-client destructive testing.
6. Confirm Production Mailgun domain is `notify.callercore.com`.
7. Confirm production Gmail encryption key is strong and stable.
8. Confirm the production Vercel alert webhook secret matches the configured production webhook.
9. Record any intentional Production+Preview shared secret and why it is safe.
