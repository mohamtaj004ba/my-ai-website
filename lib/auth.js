...
async function destroySessionToken(token){
  if(!token)return;
  await Promise.allSettled([kv.del(sessionKey(token)),kv.del('session:'+token)]);
}
async function getSession(req){
  const token=parseCookies(req).cc_session;if(!token)return null;
  const s=await readSessionToken(token);if(!s||!s.workspaceId)return null;
...
async function createSession(res,payload){
  ...
  await kv.set(sessionKey(token),{...payload,createdAt:Date.now()},{ex:SESSION_TTL});
