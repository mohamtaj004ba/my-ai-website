...
async function getSession(req){const token=parseCookies(req).cc_session;if(!token)return null;const s=await kv.get('session:'+token);return s&&s.workspaceId?s:null}
