...
// Multi-key mutations and their admin audit entry share one Redis commit.
// Decode/encode the audit history before touching any campaign or index keys.
const CONFIG_MULTI_COMPARE_AND_AUDIT=`
local count=tonumber(ARGV[1])
if not count or count<1 or #KEYS~=count+1 then return -3 end
for i=1,count do
  local current=redis.call('GET',KEYS[i]) or ''
...
return 1
`;
async function compareAndSetWithAudit(kv,updates,auditKey,event,{deleteKeys=[]}={}){
  if(!updates.length||!auditKey||!event)throw new Error('Configuration updates and audit event are required');
  const keys=updates.map(x=>x.key);
  if(new Set([...keys,auditKey]).size!==keys.length+1||deleteKeys.some(key=>!keys.includes(key)))throw new Error('Invalid configuration or audit keys');
  const args=[String(updates.length),...updates.flatMap(x=>[x.before==null?'':JSON.stringify(x.before),deleteKeys.includes(x.key)?'__CALLERCORE_DELETE__':JSON.stringify(x.after)]),JSON.stringify(event)];
  const result=Number(await kv.eval(CONFIG_MULTI_COMPARE_AND_AUDIT,[...keys,auditKey],args));
  if(result===-1)throw new Error('Audit history is malformed');
  if(result===-2)throw new Error('Audit event could not be serialized');
  if(result!==0&&result!==1)throw new Error('Configuration and audit transaction could not be confirmed');
  return result===1;
}
module.exports.compareAndSetWithAudit=compareAndSetWithAudit;
module.exports.CONFIG_MULTI_COMPARE_AND_AUDIT=CONFIG_MULTI_COMPARE_AND_AUDIT;
