...
function conversationActivity(item={}){
  const base=Math.max(timestamp(item.updatedAt),timestamp(item.createdAt),timestamp(item.at),timestamp([item.date,item.time].filter(Boolean).join(' ')));
...
function conversationSummary(item={}){
  ...
  return {id:source.id,name:source.name,phone:source.phone,address:source.address,status:source.status,last:source.last,time:source.time,createdAt:source.createdAt,updatedAt:source.updatedAt,messageCount:safeMessages.length,activityAt:conversationActivity(source)};
}
...
module.exports={DEFAULT_LIMIT,MAX_LIMIT,conversationActivity,conversationSummary,paginateConversations,paginateMessages};
