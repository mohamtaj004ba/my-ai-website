...
async function recordFinanceSnapshot(kv,initialHistory,snapshot,{attempts=3,seedHistory=[]}={}){
  ...
  for(let attempt=0;attempt<maxAttempts;attempt++){
    ...
    const history=Array.isArray(current)?current:(attempt===0&&Array.isArray(seedHistory)?seedHistory:[]);
