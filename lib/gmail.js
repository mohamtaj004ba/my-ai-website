...
async function listInbox(adminEmail,{maxResults=40,query='newer_than:30d'}={}){
  ...
  const loadThread=async ref=>{
    ...
    return {id:t.id,historyId:t.historyId||'',messages,last,subject:last.subject||'(no subject)',lastAt:last.at||0,unread:messages.some(m=>m.unread&&m.direction==='inbound')};
  };
