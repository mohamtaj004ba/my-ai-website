...
async function gmailFetch(adminEmail,path,opts={}){
  ...
  for(let attempt=0;attempt<3;attempt++){
    ...
    const delay=retryAfter>0?retryAfter*1000:400*(attempt+1);
...
function threadCacheKey(adminEmail,threadId){
  const who=crypto.createHash('sha256').update(String(adminEmail||'').toLowerCase()).digest('hex').slice(0,24);
  return 'gmail:thread:'+who+':'+String(threadId||'');
}
async function listInbox(adminEmail,{maxResults=40,query='newer_than:30d'}={}){
  ...
  const refs=list.threads||[],threads=[];
  const loadThread=async ref=>{
    const cacheKey=threadCacheKey(adminEmail,ref.id),cached=await kv.get(cacheKey);
    if(cached&&cached.thread&&ref.historyId&&String(cached.historyId||'')===String(ref.historyId))return cached.thread;
  ...
  for(let i=0;i<refs.length;i+=2){
    const batch=await Promise.all(refs.slice(i,i+2).map(loadThread));
    threads.push(...batch);
    if(i+2<refs.length)await new Promise(r=>setTimeout(r,220));
  }
...
async function markThreadRead(adminEmail,threadId){
  if(!threadId)return;
  const result=await gmailFetch(adminEmail,'/threads/'+encodeURIComponent(threadId)+'/modify',{method:'POST',body:JSON.stringify({removeLabelIds:['UNREAD']})});
  await kv.del(threadCacheKey(adminEmail,threadId));
  return result;
}
async function listAliases(adminEmail){
  const data=await gmailFetch(adminEmail,'/settings/sendAs');
  const aliases=[];
  for(const a of data.sendAs||[]){
    let inboundSeen=false;
    try{
      const q='to:'+String(a.sendAsEmail||'');
      const r=await gmailFetch(adminEmail,'/messages?maxResults=1&q='+encodeURIComponent(q));
      inboundSeen=Array.isArray(r.messages)&&r.messages.length>0;
    }catch(_){}
    aliases.push({
      ...
      verificationStatus:a.verificationStatus||'accepted',
      inboundSeen
    });
  }
  return aliases;
}
async function sendMessage(adminEmail,{to,subject,body,threadId='',inReplyTo='',references='',from=''}){
  const lines=[];if(from)lines.push('From: '+from);lines.push('To: '+to,'Subject: '+subject,'MIME-Version: 1.0','Content-Type: text/plain; charset="UTF-8"');
  if(inReplyTo)lines.push('In-Reply-To: '+inReplyTo);
  if(references)lines.push('References: '+references);
  lines.push('',body);
  const payload={raw:b64urlEncode(lines.join('\r\n'))};if(threadId)payload.threadId=threadId;
  const result=await gmailFetch(adminEmail,'/messages/send',{method:'POST',body:JSON.stringify(payload)});
  if(threadId)await kv.del(threadCacheKey(adminEmail,threadId));
  return result;
}
