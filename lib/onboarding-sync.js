...
async function syncCompletedOnboarding(record){
  ...
  const settings={
    ...
    businessPhone:clean(i.phone,40)||workspace.contactPhone||'',
  ...
  const baseAgent={
    ...
    handlingInstructions:clean(i.callHandling,500),
  ...
  generated.knowledge=baseAgent.knowledge;generated.handlingInstructions=baseAgent.handlingInstructions;generated.callHandling=baseAgent.callHandling;generated.addressSharing=baseAgent.addressSharing;generated.source='smart_onboarding';generated.draftStatus='needs_admin_review';generated.updatedAt=now;
  ...
  const nextWorkspace={
    ...
    ownerEmail:workspace.ownerEmail,contactPhone:settings.businessPhone||workspace.contactPhone||'',industry:settings.industry||workspace.industry,
    status:workspace.status==='suspended'?'suspended':'onboarding',updatedAt:now
  };
  await kv.set(wsKey,nextWorkspace);

  const priorOnboarding=await kv.get('onboarding:workspace:'+id)||{};
  const onboardingState={
    ...priorOnboarding,
    workspaceId:id,status:'intake_complete',completionPercent:100,intakeCompletedAt:record.intakeCompletedAt||now,
    agreementVersion:record.agreementVersion||priorOnboarding.agreementVersion||'',
    agreementSignedAt:record.agreementSignedAt||priorOnboarding.agreementSignedAt||null,
    agreementSignedName:record.agreementFullName||priorOnboarding.agreementSignedName||'',
    website:settings.website||'',websiteScan:record.websiteScan||null,
    checklist:{
      ...(priorOnboarding.checklist||{}),
      ...
      adminReview:!!priorOnboarding.checklist?.adminReview,
      testCall:!!priorOnboarding.checklist?.testCall,
      clientApproval:!!priorOnboarding.checklist?.clientApproval,
      live:!!priorOnboarding.checklist?.live
    },
    updatedAt:now
  };
