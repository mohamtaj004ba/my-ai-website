...
function makePrimaryDataset(){
  ...
  // One long fictional history exercises bounded rendering in authenticated QA.
  if(conversations[0]){
    const thread=conversations[0],history=Array.from({length:120},(_,i)=>({who:'Service team',dir:'system',text:'Historical service update '+(i+1)+': request history retained for the team.',at:thread.createdAt-(120-i)*3600000}));
    thread.messages=[...history,...thread.messages];
  }
