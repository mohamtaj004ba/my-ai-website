...
const ADMIN_CLIENTS=[
  ...
  ['Inland Garage Door','Starter','active','active',292,'Garage Door','live'],
...
function adminWorkspace(idBase,index,now=Date.now()){
  ...
  const phoneReady=!['building','review'].includes(scenario);
...
function adminSeedCalls(index,workspaceName){
  const count=[32,58,41,28,65,8,36,21,0,15][index]??(24+index*9),items=[];
...
function adminSeedLeads(index){
  ...
  const counts=[18,27,14,9,33,2,11,6,0,8],items=[];
