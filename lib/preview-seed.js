...
const HVAC_REASONS=[
  ...
  ['AC not cooling','Booked',380],
  ['Seasonal HVAC tune-up','Booked',189],
  ...
  ['Indoor air quality question','Follow-up',850],
  ['Ductwork estimate','Qualified',2400],
  ['Water heater replacement','Booked',1850],
  ['After-hours no heat','Booked',525],
  ['Maintenance plan question','Follow-up',240]
];
...
function makePrimaryDataset(){
  ...
  for(let day=0;day<60;day++){
    const daily=18+((day*5)%5); // 18-22/day, ~20 average
    for(let j=0;j<daily;j++){
      const i=callIndex++,person=nameFor(i),reasonRow=pick(HVAC_REASONS,i*3+day),reason=reasonRow[0];
      ...
      const id='call_'+String(i+1).padStart(4,'0'),city=pick(CITY_DATA,i+day),address=(110+i%7800)+' '+pick(STREETS,i*3+day)+', '+city[0]+', '+city[1]+' '+city[2];
      calls.push({
        id,caller:person,phone:phoneFor(i),address,reason,duration:outcome==='Missed'?'0:18':durationFor(i),outcome,
        ...
        summary:outcome==='Missed'
          ...
          :person+' called about '+reason.toLowerCase()+'. Maya captured the service need, location, preferred timing, and contact details'+(outcome==='Booked'?' and confirmed the requested service window.':'.'),
        qualification:{Intent:/Booked|Qualified/.test(outcome)?'High':'Medium',Service:reason,Timeline:(i%3===0?'Today':i%3===1?'This week':'This month'),Value:value?('$'+value.toLocaleString('en-US')):'—'},
        transcript:outcome==='Missed'?[['CallerCore','Missed call detected. Team follow-up created.']]:[
          ...
          ['Maya',outcome==='Booked'?'I have your information and requested service window. The team will see it right away.':'I have everything I need. I will send this to the service team for follow-up.'],
          [person.split(' ')[0],'Great, thank you.']
        ]
      });
      if(!['Missed'].includes(outcome)&&((i+day)%3!==0)){
        const stage=outcome==='Booked'?'Appointment':outcome==='Qualified'?'Qualified':pick(STAGES,leadIndex*2+day);
        const lead={id:'lead_'+String(++leadIndex).padStart(4,'0'),name:person,phone:phoneFor(i),address,service:reason,value:Math.max(180,value||350),stage,source:'AI call',age:day===0?((j+4)+'m'):(day+'d'),createdAt};
        leads.push(lead);
        if((i+day)%2===0){
          conversations.push({
            id:'conv_'+id,name:person,phone:phoneFor(i),address,status:stage==='Appointment'?'Active':stage==='Lost'?'Closed':'Needs follow-up',
            last:stage==='Appointment'?'Service window confirmed with the customer.':'Team follow-up requested after the call.',
            time:timeLabel(createdAt),createdAt,messages:[
              ...
              {who:person.split(' ')[0],text:stage==='Appointment'?'Perfect, thank you.':'Thanks — please have someone follow up with me.',dir:'in',at:createdAt+180000}
            ]
          });
        }
        if(stage==='Appointment'&&apptIndex<180){
          const apptAt=createdAt+(1+(i%5))*86400000;
          appointments.push({id:'appt_'+String(++apptIndex).padStart(4,'0'),name:person,phone:phoneFor(i),date:dateLabel(apptAt),time:timeLabel(apptAt),service:reason,status:day>5?'Completed':'Confirmed',source:'Maya',createdAt:apptAt});
        }
      }
    }
  }
