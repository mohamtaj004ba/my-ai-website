...
function makePrimaryDataset(){
  ...
  for(let day=0;day<60;day++){
    ...
    for(let j=0;j<daily;j++){
      ...
      const id='call_'+String(i+1).padStart(4,'0');
      calls.push({
        id,caller:person,phone:phoneFor(i),reason,duration:outcome==='Missed'?'0:18':durationFor(i),outcome,
        ...
        transcript:outcome==='Missed'?[['CallerCore','Missed call detected. Team follow-up created.']]:[
          ...
          ['Maya','Absolutely. I can help with that. Let me get a few details so the team can take care of you.']
        ]
      });
      if(!['Missed'].includes(outcome)&&((i+day)%3!==0)){
        ...
        const lead={id:'lead_'+String(++leadIndex).padStart(4,'0'),name:person,phone:phoneFor(i),service:reason,value:Math.max(180,value||350),stage,source:'AI call',age:day===0?((j+4)+'m'):(day+'d'),createdAt};
        leads.push(lead);
        if((i+day)%2===0){
          conversations.push({
            id:'conv_'+id,name:person,phone:phoneFor(i),status:stage==='Appointment'?'Active':stage==='Lost'?'Closed':'Needs follow-up',
