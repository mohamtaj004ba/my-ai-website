...
async function rateLimit({scope,identifier,limit,windowSeconds,failClosed=false}){
  ...
  try{
    ...
    return {limited:!!failClosed,count:0,limit:max,retryAfter:Math.min(ttl,60),degraded:true,unavailable:!!failClosed};
  }
}
