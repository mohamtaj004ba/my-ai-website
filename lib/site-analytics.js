...
local eventsType=redis.call('TYPE',KEYS[1]).ok
if eventsType~='none' and eventsType~='list' then return -1 end
if ARGV[1]=='1' and ARGV[5]=='1' then
  local directoryType=redis.call('TYPE',KEYS[3]).ok
  if directoryType~='none' and directoryType~='list' then return -2 end
end
...
async function recordSiteEvent(raw={},req=null){
  ...
  for(let attempt=0;attempt<4;attempt++){
    ...
    if(snapshot&&(typeof snapshot!=='object'||Array.isArray(snapshot)||String(snapshot.id||'')!==event.sessionId||!Number.isFinite(Number(snapshot.firstAt))||Number(snapshot.firstAt)<=0||(snapshot.pages!==undefined&&!Array.isArray(snapshot.pages))||!Number.isFinite(Number(snapshot.activeMs||0))||Number(snapshot.activeMs||0)<0||!Number.isFinite(Number(snapshot.events||0))||!Number.isInteger(Number(snapshot.events||0))||Number(snapshot.events||0)<0||!Number.isFinite(Number(snapshot.lastAt||0))))throw new Error('Website session is malformed');
    let session=null;
    if(key){
    ...
    if(saved===-1)throw new Error('Website event directory is malformed. No event was recorded');
    if(saved===-2)throw new Error('Website session directory is malformed. No event was recorded');
...
if indexType~='none' and indexType~='list' then return -3 end
...
if ARGV[10]=='1' then
  auditKey=auditKey+1
  local auditType=redis.call('TYPE',KEYS[auditKey]).ok
  if auditType~='none' and auditType~='string' then return -1 end
...
async function upsertWebsiteProspectAttempt(raw={}){
  ...
  const next={...old,id,
    ...
    convertedAt:Number(old.stage==='converted'&&old.convertedAt?old.convertedAt:raw.convertedAt!==undefined?raw.convertedAt:(old.convertedAt||0))||null,
  ...
  if(result===-3)throw new Error('Prospect directory is malformed. No lead was created');
