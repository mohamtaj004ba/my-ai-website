...
async function upsertWebsiteProspect(raw={}){
  ...
  if(snapshot&&typeof snapshot!=='object')throw new Error('Prospect record is malformed');
  const next={...old,id,
  ...
  const args=[snapshot==null?'':JSON.stringify(snapshot),JSON.stringify(next),owner==null?'':String(owner),previousOwner==null?'':String(previousOwner),nextKey?'1':'0',previousKey?'1':'0',previousKey&&String(previousOwner||'')===id?'1':'0',snapshot==null?'1':'0',id];
  ...
  if(!result)throw new Error('Prospect changed during save. Retry after refreshing the record');
  return next;
}
