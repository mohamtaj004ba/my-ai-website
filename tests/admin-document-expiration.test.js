...
test('documents table, review counts, and navigation badge use consistent expiry rule',()=>{
  assert.equal(source.split('companyDocumentIsExpired(x)').length-1,3);
  ...
  assert.match(render,/company\.filter\(x=>.*companyDocumentIsExpired\(x\)/);
  assert.match(render,/const expired=companyDocumentIsExpired\(x\)/);
