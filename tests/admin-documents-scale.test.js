...
function fixture(count){
  ...
  const kv={get:async key=>{
    ...
    if(key.startsWith('onboarding:workspace:'))return {agreementSignedAt:1,agreementVersion:'v1',agreementSignedName:'Owner'};
  ...
  return {context,reads,res,run:()=>vm.runInContext('adminDocuments(req,res)',context)};
}
