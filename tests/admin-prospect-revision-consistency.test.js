...
function backend(body,{stored={id:'lead-1',name:'Old',stage:'new',email:'old@example.test',updatedAt:10},commit=true,now=10}={}){
  let code,result,calls=0,payload,plainWrites=0;
  const context=vm.createContext({
    requireAdmin:async()=>({email:'admin@example.test'}),req:{body},res:{status(n){code=n;return this},json(x){result=x;return x}},
    kv:{get:async()=>stored,set:async()=>{plainWrites++}},
  ...
  return {run:async()=>{await vm.runInContext('adminWebsiteProspectUpdate(req,res)',context);return {code,result,calls,payload,plainWrites}}};
}
